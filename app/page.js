...
function createLiveDiagnostics() {
  return {
    ...
    geminiLastReconnectMs: null,
    translationLatencyMs: null,
    reconnects: 0,
    geminiResumeReady: false,
    geminiGoAwayTimeLeft: '—',
    lastError: '',
    lastErrorAt: '',
    updatedAt: '',
  };
    ...
    >
      ...
      >
        ...
        <DiagnosticsMetric
          label="Última retoma"
          value={metricText(
            diagnostics.geminiLastReconnectMs,
            ' ms',
            0
          )}
        />
        ...
        <DiagnosticsMetric
          label="GoAway"
          value={
            diagnostics.geminiGoAwayTimeLeft ||
            '—'
          }
        />
...
export default function Home() {
  ...
  const geminiResumeHandleRef =
    useRef('');

  const geminiReconnectPromiseRef =
    useRef(null);

  const geminiShouldRunRef =
    useRef(false);

  const geminiConnectionSerialRef =
    useRef(0);

  const geminiTargetLanguageRef =
    useRef('');
  ...
  function resetLiveDiagnostics() {
    ...
    geminiResumeHandleRef.current =
      '';

    geminiReconnectPromiseRef.current =
      null;

    geminiShouldRunRef.current =
      false;

    geminiConnectionSerialRef.current =
      0;

    geminiTargetLanguageRef.current =
      '';
  ...
  async function requestGeminiSession(
    targetLanguage
  ) {
  ...
  async function openGeminiConnection({
    session,
    resumeHandle = '',
    isReconnect = false,
    previousWs = null,
    ...
    ws.__aloeConnectionSerial =
      connectionSerial;

    ws.__aloeExpectedClose =
      false;
      ...
      (resolve, reject) => {
        ...
        ws.onopen = () => {
          ...
          try {
            ...
            console.log(
              resumeHandle
                ? 'Setup Gemini de retoma enviado.'
          ...
          async (event) => {
            try {
              ...
              if (
                  ...
                  (previous) => ({
                    ...
                    geminiLastReconnectMs:
                      isReconnect
                        ...
                        : previous.geminiLastReconnectMs,
                    reconnects:
                      isReconnect
                        ? previous.reconnects +
                          1
                ...
                setStatus(
                  isReconnect
                    ? 'Chamada ligada — tradução Gemini retomada'
                ...
                if (
                  ...
                  try {
                    previousWs.__aloeExpectedClose =
                      true;
  ...
  async function reconnectGemini(
    ...
    const reconnectPromise =
      (async () => {
        ...
        setStatus(
          'Chamada ligada — a renovar sessão Gemini…'
        );
        ...
        try {
          ...
          throw new Error(
            'Ainda não existe handle de session resumption.'
          );
        } catch (
          resumeError
        ) {
          console.warn(
            'Retoma Gemini não foi possível; a iniciar sessão nova:',
            reason,
            resumeError
          );
        ...
        .catch(
          (reconnectError) => {
            const message =
              `Não foi possível retomar a tradução Gemini: ${reconnectError.message}`;
